...
class Monitor extends BeanModel {
  ...
  async toPublicJSON(showTags = false, certExpiry = false) {
    ...
    if (
      ...
      const { certExpiryDaysRemaining, validCert } = await this.getCertExpiry(
        this.id
      );
        ...
        if (await Monitor.isUnderMaintenance(this.id)) {
          ...
          if (this.auth_method === "basic") {
            basicAuthHeader = {
              Authorization:
                "Basic " +
                this.encodeBase64(this.basic_auth_user, this.basic_auth_pass),
            };
          }
          ...
          if (
            process.env.UPTIME_KUMA_LOG_RESPONSE_BODY_MONITOR_ID === this.id
          ) {
          ...
          if (this.type === "http") {
            ...
            if (keywordFound === !this.isInvertKeyword()) {
              bean.msg +=
                ", keyword " + (keywordFound ? "is" : "not") + " found";
              bean.status = UP;
            } else {
              data = data.replace(/<[^>]*>?|[\n\r]|\s+/gm, " ").trim();
              if (data.length > 50) {
          ...
          if (response.code !== 1) {
            ...
            if (keywordFound === !this.isInvertKeyword()) {
              bean.status = UP;
              bean.msg = `${responseData}, keyword [${this.keyword}] ${
                keywordFound ? "is" : "not"
              } found`;
            } else {
              log.debug(
                "monitor:",
                `GRPC response [${response.data}] + ", but keyword [${
                  this.keyword
                }] is ${keywordFound ? "present" : "not"} in [" + ${
                  response.data
                } + "]"`
              );
              bean.status = DOWN;
    ...
    if (!isFirstBeat || bean.status === DOWN) {
      ...
      for (let notification of notificationList) {
        try {
          ...
          const monitorData = [
            { id: monitor.id, active: monitor.active, name: monitor.name },
          ];
  ...
  async sendCertNotificationByTargetDays(
    ...
    for (let notification of notificationList) {
      try {
        ...
        await Notification.send(
          JSON.parse(notification.config),
          `[${os.hostname()}] [${this.name}][${this.url}] ${certType} certificate ${certCN} will expire in ${daysRemaining} days`
        );
        sent = true;
      } catch (e) {
        log.error(
  ...
  static async isUnderMaintenance(monitorID) {
    ...
    for (const maintenanceID of maintenanceIDList) {
      const maintenance = await UptimeKumaServer.getInstance().getMaintenance(
        maintenanceID
      );
  ...
  validate() {
    ...
    if (this.type === "ping") {
      ...
      if (
        this.ping_count &&
        (this.ping_count < PING_COUNT_MIN || this.ping_count > PING_COUNT_MAX)
      ) {
        throw new Error(
  ...
  async makeOidcTokenClientCredentialsRequest() {
    ...
    if (this.oauthAccessToken?.expires_at) {
      log.debug(
        "monitor",
        `[${this.name}] Obtained oauth access-token. Expires at ${new Date(
